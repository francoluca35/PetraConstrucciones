...
export function Hero() {
  return (
    <section className="relative flex min-h-screen flex-col justify-end overflow-hidden">
      ...
      {/* Contenido */}
      <div className="relative z-10 flex flex-1 flex-col justify-end px-6 pb-12 pt-24 md:px-12 md:pb-16 lg:px-16 lg:pb-20">
        ...
        >
          <Link
            href="/contacto"
            className="inline-block  bg-white hover:bg-[#E5C337] px-8 py-3 text-center font-semibold text-[#1a1918] hover:text-[#283777] transition-opacity hover:opacity-90"
          >
            Contactanos
          </Link>
        </motion.div>

        {/* Estadísticas - fila centrada */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, delay: 0.3 }}
          className="flex flex-wrap items-end gap-12 md:gap-16 lg:gap-24"
        >
          {STATS.map((stat, i) => (
